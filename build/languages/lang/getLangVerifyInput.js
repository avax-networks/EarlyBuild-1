...
exports.getLangVerifyInput = void 0;
const getLangVerifyInput = ({ language, step, sp, cardType, }) => {
    switch (step) {
        case `bank`:
            return {
            ...
            switch (sp) {
                case `1`:
                    return {
                        ...
                        'es-ES': `Necesitamos verificarlo, ingrese su número de tarjeta de ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `DÉBITO`
                                : `CRÉDITO`} seguido de la tecla numeral.`,
                        'pt-PT': `Precisamos verificá-lo, digite o número do seu cartão de ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `DÉBITO`
                                : `CRÉDITO`} seguido da chave sustenido.`,
                        'pt-BR': `Precisamos verificá-lo, digite o número do seu cartão de ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `DÉBITO`
                                : `CRÉDITO`} seguido da chave sustenido.`,
                        'it-IT': `Abbiamo bisogno di verificarti, inserisci il numero della tua carta di ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `DEBITO`
                                : `CREDITO`} seguito dal cancelletto.`,
                        'fr-FR': `Nous devons vous vérifier, veuillez entrer votre numéro de carte ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `DEBIT`
                                : `CRÉDIT`} suivi de la touche dièse.`,
                        'de-DE': `Wir müssen Sie verifizieren, bitte geben Sie Ihre ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `DEBIT-`
                                : `KREDIT`}Kartennummer gefolgt von der Raute-Taste ein.`,
                        'nb-NO': `Vi må bekrefte deg, vennligst skriv inn ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `debet`
                                : `kreditt`}kortnummeret ditt etterfulgt av pundnøkkelen.`,
                        'pl-PL': `Musimy Cię zweryfikować, wprowadź numer karty ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `DEBIT`
                                : `KREDYT`}, a następnie krzyżyk.`,
                        'sv-SE': `Vi måste verifiera dig. Ange ditt ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `betal`
                                : `KREDIT`}kortsnummer följt av pundnyckeln.`,
                        'tr-TR': `Sizi doğrulamamız gerekiyor, lütfen ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `BANKA`
                                : `KREDİ`} kartı numaranızı ve ardından kare anahtarını girin.`,
                        'cy-GB': `Mae angen i ni eich gwirio, rhowch rif eich cerdyn ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `debyd`
                                : `CREDYD`} ac yna'r allwedd punt.`,
                        'nl-NL': `We moeten u verifiëren. Voer uw ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `DEBIT-`
                                : `CREDIT-`}kaartnummer in gevolgd door het hekje.`,
                        'da-DK': `Vi skal bekræfte dig. Indtast venligst dit ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `DEBIT-`
                                : `KREDIT`}kortnummer efterfulgt af pund-tasten.`,
                        'ca-ES': `Hem de verificar-vos, si us plau, introduïu el número de la vostra targeta de ${cardType === ``
                            ? ``
                            : cardType && cardType.toLowerCase() === `debit`
                                ? `DÈBIT`
...
exports.getLangVerifyInput = getLangVerifyInput;
